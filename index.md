---
title: Home
description: Research homepage of Shigeru Fujita (DVM)
---

<link rel="stylesheet" href="./assets/style.css">

<header>
  <h1>Shigeru Fujita, DVM</h1>
  <nav>
  <a href="./profile.html">Profile</a> |
  <a href="./research.html">Research</a> |
  <a href="./publications.html">Publications</a> |
  <a href="./cv.html">CV</a> |
  <a href="./blog/2026-03-06-welcome.html">Blog</a>
  </nav>

</header>

> **Field**: Virology / Veterinary Medicine  
> **Affiliation**: Division of Systems Virology, Dept. Microbiology & Immunology,  
> The Institute of Medical Science, The University of Tokyo（PhD candidate, 2022–2026）  
> **Next**: Postdoctoral Fellow（2026‑05〜）, Institute for Medical Virology and Epidemiology of Viral Diseases, University Hospital Tübingen  
> **Keywords**: coronavirus spillover, receptor tropism, antigenicity, neutralizing antibodies

## News
- 2026‑05: JSPS Overseas Research Fellow として Univ. Tübingen に着任予定  
- 2024‑06: EBioMedicine に筆頭著者論文（BANAL‑20‑236 のウイルス学的性状）掲載  
- 2023‑09: Journal of Virology に筆頭著者論文（Rhinolophus bat ACE2 トロピズム解析）掲載

## Research Overview
私は獣医師・ウイルス研究者として、**ウイルスの免疫回避機構**と**動物由来ウイルスのヒトへの適応（スピルオーバー）**を主な研究テーマとしています。

ウイルス抗原性と中和抗体の研究では、ネコカリシウイルスに対する広域中和抗体のエピトープを同定し（Fujita et al., *Virus Research*, 2022）、SARS‑CoV‑2では特定変異が治療抗体の効果を低下させる分子機序を明らかにしました（Fujita et al., *Viruses*, 2022）。また、新規変異株のウイルス学的特性評価やワクチン効果の検証にも貢献しました。

コロナウイルスのスピルオーバーについての研究では、コウモリ由来ウイルスの受容体使用性を解析し、感受性を決定するACE2領域を特定しました（Fujita et al., *Journal of Virology*, 2023）。さらに、感染性ウイルスを用いてコウモリ由来ウイルスのヒト気道での低増殖性や腸管での効率な複製を示し（Fujita et al., *EBioMedicine*, 2024）、この成果は *Nature News* でも紹介されました。加えて、ベトナムでのコウモリ調査とウイルス遺伝子サーベイランスにも参加し、自然界におけるウイルスのサーベイランスにも取り組んでいます。

これらの研究を通じ、効果的なワクチン・治療薬開発に資する基礎科学の構築と、次のパンデミックへの備えに寄与しています。

<br />
My research as a veterinarian and virologist centers on two major questions: **how viruses evade immune protection** and **how animal viruses adapt to infect humans**. By integrating molecular virology, antibody analysis, and field studies, I aim to generate insights that support the development of effective vaccines and therapeutics and strengthen preparedness for future pandemics.

In the area of virus antigenicity and neutralizing antibodies, I identified an epitope targeted by a broadly neutralizing monoclonal antibody against feline calicivirus, located in a region of low mutation frequency rather than the known hypervariable site (Fujita et al., *Virus Research*, 2022). In SARS‑CoV‑2, I demonstrated that a specific spike substitution markedly reduces the efficacy of cilgavimab through steric hindrance at the antibody–spike interface (Fujita et al., *Viruses*, 2022). I have also contributed to characterizing emerging variants and assessing vaccine effectiveness (Fujita et al., *J. Infect. Dis.*, 2023; Kimura et al., *Cell*, 2022; Ito et al., *Nat. Commun.*, 2023), including work associated with a highly cited paper on the XBB variant.

My research on coronavirus spillover and zoonotic risk includes identifying key ACE2 regions that determine susceptibility to bat‑borne coronaviruses (Fujita et al., *Journal of Virology*, 2023). Using infectious viruses, I further showed that bat‑derived coronaviruses differ markedly from SARS‑CoV‑2 in cellular and tissue tropism and replicate poorly in the human respiratory tract (Fujita et al., *EBioMedicine*, 2024). This work has been featured in **Nature News** and highlighted by the Institute of Medical Science, The University of Tokyo. I also participate in fieldwork in Vietnam, conducting bat capture and on‑site viral genetic surveillance to uncover the virological diversity in natural reservoirs.

## Contact
- Email: fujitashigeru0701(AT)g.ecc.u‑tokyo.ac.jp  
- ORCID: https://orcid.org/0000‑0001‑9008‑2123
